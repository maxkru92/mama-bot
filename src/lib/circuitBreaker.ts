...
export function validateCircuitBreakerConfig(config: any): boolean {
  ...
  if (config.halfOpenMaxCalls !== undefined && typeof config.halfOpenMaxCalls !== 'number')
    return false

  if (config.maxFailures !== undefined && (config.maxFailures < 1 || config.maxFailures > 100))
    return false
  if (config.timeout !== undefined && (config.timeout < 0 || config.timeout > 3600000)) return false
  if (
    config.halfOpenMaxCalls !== undefined &&
    (config.halfOpenMaxCalls < 1 || config.halfOpenMaxCalls > 10)
  )
    return false
